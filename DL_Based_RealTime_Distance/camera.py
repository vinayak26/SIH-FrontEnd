...
def write(bboxes, img, classes, colors):
    ...
    delta_dist.append(distance)
    final_dist[0] += abs(delta_dist[1] - delta_dist[0])
    speed = final_dist[0]
    delta_dist[0] = delta_dist[1]
    delta_dist.pop(1)

    feedback = "Distance: {:.5f}m".format(final_dist[0] * 0.3)
    file = open('distance', 'w')
    file.write(final_dist)
    file.close()
...
class ObjectDetection:
    def __init__(self, id):
        ...
        self.width = 640  # 640#1280
        self.height = 360  # 360#720
